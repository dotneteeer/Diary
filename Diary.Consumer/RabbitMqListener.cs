...
public class RabbitMqListener : BackgroundService
{
    ...
    public RabbitMqListener(IOptions<RabbitMqSettings> options, ILogger logger)
    {
        _options = options;
        _logger = logger;
        var factory = new ConnectionFactory { HostName = options.Value.HostName, Port = options.Value.Port };
    ...
    //create exchange, queue and bind them with names in appsettings.json
}